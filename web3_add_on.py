...
#}
"""
    blender addon prototype for web3
    1. post to minds
    2. spawn monkey
...
class PANEL_PT_TestPanel(bpy.types.Panel):
    ...
    bl_region_type = "UI"
    bl_context = 'Jolly\'s Bag \'O Tricks'
    ...
    def draw(self, context):
        ...
        # post to Minds
        row1 = layout.row()
        row1.label(text="MINDS", icon="OUTLINER_DATA_LIGHT")
        row1.operator(MindsOperator.bl_idname, text=MindsOperator.bl_label)

         # Spawn monkey
        row2 = layout.row()
        row2.label(text="MONKEY", icon="MONKEY")
        row2.operator(MonkeyOperator.bl_idname, text=MonkeyOperator.bl_label)
